# BrowserWindow 浏览器窗口
`BrowserWindow` 这个类可以让你创建一个新的窗口。例如
...
win.show();
```
你还可以使用 [Frameless Window](frameless-window.md) API 创建一个不是 chrome 样式的窗口
...
* `options` Object
  * `width` Integer - 窗口的像素宽度.默认值为 `800`.
  * `height` Integer - 窗口的像素高度. 默认值为 `600`.
